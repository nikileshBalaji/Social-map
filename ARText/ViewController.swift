    ...
    @objc func handleTap(rec: UITapGestureRecognizer){
        DispatchQueue.main.async {
            if self.key {
                ...
                if let d = getUserData(withKey: "data") as? [Data] {
                    self.assets = d
                    self.collectionView.reloadData()
                }
                if let e = getUserData(withKey: "eventName") as? String {
                    self.eventNameLabel.text = e
                }
            }
...
//                self.inputContainerViewBottom.constant = keyboardHeight
    ...
    @IBAction func userClickedBackButton(_ sender: Any) {
        collectionView.isHidden = true
        backButton.isHidden = true
        eventNameLabel.isHidden = true
    }
...
extension ViewController : UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell?.userSelectedImage.image = UIImage(data: assets[indexPath.row])
